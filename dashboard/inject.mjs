#!/usr/bin/env node
// Meridian Dashboard Data Synthesizer
...
export async function synthesize(data) {
  ...
  // === Browser Use HUMINT ===
  const buData = data.sources['BrowserUse-HUMINT'] || {};
  const humint = {
    findings: (buData.findings || []).slice(0, 15).map(f => ({
      agent: f.agent, text: f.text?.substring(0, 300),
      severity: f.severity || 'medium', category: f.category || 'geopolitical',
    })),
    agents: (buData.agents || []).map(a => ({ name: a.name, status: a.status })),
    disabled: buData.disabled || false,
    cached: buData.cached || false,
    summary: buData.summary || '',
  };

  // === Missile & Air Defense ===
  const missileData = data.sources['Missile/AirDef'] || {};
  const missileKeywords = ['missile','rocket','intercept','air defense','air raid','air alert',
    'ATACMS','Patriot','Iron Dome','S-300','S-400','THAAD','Shahed','Iskander','Kalibr',
    'Kinzhal','cruise missile','ballistic','Houthi','drone attack','UAV strike','HIMARS',
    'shelling','barrage','salvo','incoming','launch detected','air alarm','explosion reported',
    'hypersonic','Zircon','Storm Shadow','SCALP'];

  // Cross-correlate Telegram with missile keywords
  const missileOsint = [...tgUrgent, ...tgTop].filter(p => {
    if (!p.text) return false;
    const lower = p.text.toLowerCase();
    return missileKeywords.some(kw => lower.includes(kw.toLowerCase()));
  }).map(p => ({
    channel: p.channel, text: p.text?.substring(0, 200),
    date: p.date, views: p.views,
    matchedKeywords: missileKeywords.filter(kw => p.text.toLowerCase().includes(kw.toLowerCase())),
  }));

  // Cross-correlate ACLED for shelling/missile events
  const missileAcled = (acled.deadliestEvents || []).filter(e =>
    e.type && /shell|missile|bomb|air.*strike|rocket|artillery|drone/i.test(e.type)
  );

  const missileEvents = (missileData.likelyExplosions || []).concat(missileData.suspicious || []);
  const missile = {
    totalAnomalies: missileData.totalAnomalies || 0,
    likelyExplosions: (missileData.likelyExplosions || []).slice(0, 15),
    suspicious: (missileData.suspicious || []).slice(0, 10),
    byZone: missileData.byZone || {},
    testSiteActivity: missileData.testSiteActivity || [],
    osintCorrelation: missileOsint.slice(0, 10),
    acledCorrelation: missileAcled.slice(0, 10),
    totalCorrelated: missileOsint.length + missileAcled.length + missileEvents.length,
    signals: missileData.signals || [],
    testSites: missileData.testSites || [],
  };
  ...
  const V2 = {
    ...
    who, fred, energy, bls, treasury, gscpi, defense, noaa, epa, acled, gdelt, space, missile, humint, health, news,
